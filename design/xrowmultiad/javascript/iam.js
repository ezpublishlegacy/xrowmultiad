...
var iom = iom || (function () {
  var dummySite = "dummy",
      ...
      eventList = ["","inst", "open", "clse", "play", "stop", "fowa", "bakw", "recd", "paus", "forg", "bakg", "dele", "refr", "view", "alve"],
      LSOBlacklist = [],
      checkEvents = 1,
      tb = 0,
      sv = 1,
  ...
  function enableEvents() {
    if (tb && (typeof result.tb == "undefined" || result.tb) && !eventsEnabled){
      eventsEnabled = 1;
  ...
  function event(event) {
    ...
    if (inited && !isDoNotTrack() && (!checkEvents || (checkEvents && arrayContains(eventList, event)))) {
      ...
      if ( !(arrayContains(LSOBlacklist, result.st)) && ((/iPhone/.test(window.navigator.userAgent) || /iPad/.test(window.navigator.userAgent)) && (/OS\s[3-6]/.test(window.navigator.userAgent)) && /Safari/.test(window.navigator.userAgent) && !(/Chrome/.test(window.navigator.userAgent))) ) {
        baseUrl = baseUrlLSO;
        mode = 3;
  ...
  function forwardToOldSZM() {
    if (result.mg === "yes" && !window.IVW && !document.IVW) {
